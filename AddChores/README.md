# Add Chores - Roommates App Chore Management

## Overview

`chores` is a Go package that provides a Google Cloud Function for creating and managing chores within the Roommates App.  
It allows users to add chores to a group in Firestore, including details such as due dates, frequency, assignee, and status.

### Key features include:

- Add a chore to a specific group in Firestore.  
- Store key details such as name, description, due date, frequency, assignee, and status.  
- Automatically set default fields (`created_at`, `created_by`, `chore_status`, `completed_at`).  
...
```bash
go get -u github.com/your-username/roommates-cloud-functions/chores
```
...
**Example**:
```bash
curl -X POST "https://REGION-PROJECT_ID.cloudfunctions.net/AddChoreHandler" \
  -H "Content-Type: application/json" \
  -d '{
    "user_id": "user123",
    "group_id": "group456",
    "chore_name": "Take out trash",
    "chore_details": "Blue and black bins by 7pm",
    "chore_due_date": "2025-10-02",
    "chore_frequency": "weekly",
    "chore_assignee": "user789"
  }'
```
...
3. **Deploy the Function**  
   Ensure the Go file is in the same directory you are deploying from. Use:
    ...
    ```bash
    gcloud functions deploy AddChoreHandler \
      --gen2 \
      --runtime go122 \
      --trigger-http \
...
2. **Test the API locally**:
    ...
    curl -X POST "http://127.0.0.1:8080" \
      -H "Content-Type: application/json" \
      -d '{"user_id":"user123","group_id":"group456","chore_name":"Laundry","chore_due_date":"2025-10-03","chore_frequency":"weekly"}'
    ```
